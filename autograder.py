...
def test_conv_gold_test():
    ...
    process = subprocess.run(['make', 'conv_gold_test'], 
                         stdout=subprocess.PIPE, 
                         universal_newlines=True)
    ...
    process = subprocess.run(['make', 'run_conv_gold_test'], 
                        stdout=subprocess.PIPE, 
                        universal_newlines=True)
...
def test_conv_gold_tiled_test():
    ...
    process = subprocess.run(['make', 'conv_gold_tiled_test'], 
                         stdout=subprocess.PIPE, 
                         universal_newlines=True)
    ...
    process = subprocess.run(['make', 'run_conv_gold_tiled_test'], 
                        stdout=subprocess.PIPE, 
                        universal_newlines=True)
...
def test_fifo_tb():
    ...
    return run_process(['vcs', '-full64', '-sverilog', '-timescale=1ns/1ps', '-debug_access+pp', 'tests/fifo_tb.sv', 'verilog/fifo.v', 'verilog/SizedFIFO.v'])
